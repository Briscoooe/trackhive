...
export default function PlaylistSearch() {
  ...
  const [searchResponse, setSearchResponse] = useState<SpotifyPlaylistSearchResponse | null>();
  useEffect(() => {
    ...
    return (
      <div className={'flex flex-col space-y-2 w-full'}>
        <Input value={searchText} onChange={(e) => setSearchText(e.target.value)} placeholder={'Search for a playlist'}/>
        {searchResponse && searchResponse.items.map((playlist, index) => (
